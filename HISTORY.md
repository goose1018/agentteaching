...
12. **八次修正：公式渲染统一入口**
   - 现象：模型有时输出 `$...$`，有时输出 `\(...\)`、`\[...\]`，有时漏掉 `$` 或吐出 `{1}{2}`、`cdot` 等半截 LaTeX，导致页面一会儿正常、一会儿走形。
   - 修复：`Tex` 组件升级为统一归一化入口，支持 `$...$`、`\(...\)`、`\[...\]`，并自动兜底常见裸公式片段（`\frac{...}{...}`、`x=...`、`\sqrt`、`\sin` 等）。老师 prompt 也明确要求只使用行内 `$...$`，不要使用 `\(...\)`、`\[...\]` 或 `$$...$$`。
   - 边界：完全自然语言形式如 `1/2 m v ^ 2` 仍不一定可自动识别；长期应在后端做更强的 math normalization。
